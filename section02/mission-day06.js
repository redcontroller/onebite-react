...
function splitAndCombine(arr) {
    ...
    const frontHalf = arr.slice(0, halfIndex); // [0,1,2,3]
    const backHalf = arr.slice(halfIndex); // [4,5,6,7]
    // return [...frontHalf, ...backHalf];
    // return backHalf.concat(frontHalf);
    backHalf.push(...frontHalf);
    return backHalf;
}
...
function printClickEventLogs(logs) {
    // for (let {type, date} of logs) {
    //     type !== "click" ? '' : console.log(`${type} :: ${new Date(date).toLocaleString('ko-KR')}`);
    // }
    let clickCount = 0;
    for (let {type, date} of logs) {
        if (type === "click") {
            console.log(`${type} :: ${new Date(date).toLocaleString('ko-KR')}`);
            clickCount++;
        }
    }
    clickCount ? '': console.log("!! 클릭 이벤트는 발생하지 않았습니다 !!");
}
